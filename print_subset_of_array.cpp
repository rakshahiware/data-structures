#include<iostream>
using namespace std;
int subset(int input[],int n,int output[],int m)
{
  if(n==0){         //base case
   for(int i=0;i<m;i++){
      cout<<output[i]<<" ";
  ...
  subset(input+1,n-1,output,m);          //this part does not include element at 0 index ,pass output as it is
  ...
  subset(input+1,n-1,newoutput,m+1);      //this part  includes element at 0 index ,in  this case we need to pass teh element at 0 index in output array
}
void printSubset( int input[],int n){
  int m=0;                         //represents output array is empty 
  int output[m];                //we need to maintain a output array in case of print que
   subset(input,n,output,m);
}
